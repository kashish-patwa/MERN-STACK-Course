...
async function submitSignUp(req, res) {
 ...
 async function submitLogin(req,res){
  ...
  // 1) basic validation: check if all fields are filled
    if(!email || !password){
        return res.send(email).json({success: false, message:"all filled are required "})
    }
    ...
    if(!emailRegex.test(email)){
        ...
        return res.status(400).json({success: false, message:"email is not correct snytex is inncorect"})
    }
    ...
    if(!passcode.test(password)){
        return res.status(400).json({success:false,message:"check if the password is at least 8 characters long and contains at least one uppercase letter, one lowercase letter, one number, and one special character"
    })
    }
    ...
    try {
        ...
        if(!foundUser)
        {
            return res.status(404).json({success:false,message:"user does't exist please sign up first"})
        }
