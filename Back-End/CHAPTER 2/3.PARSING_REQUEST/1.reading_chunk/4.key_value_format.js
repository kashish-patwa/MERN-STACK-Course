...
//handlling /form(routes or url or end point)
      if(req.url=="/" && req.method=="GET")
      {
       ...
       //jitne bhi chunk hai un sb ko array mai store karenge
        ...
        req.on("end",()=>{
            //   joining a chunks
             let joinChunk=Buffer.concat(body)

            //  converting in the string
            let string_form=joinChunk.toString()
            console.log(string_form)

            // key value format

            let key_value_form=new URLSearchParams(string_form)
            console.log(key_value_form)
        })
      }  
})
const port=3000
